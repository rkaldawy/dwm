...
static void spawn_terminal(const Arg *arg);
...
void spawn_terminal(const Arg *arg) {
  char *command[] = {"/home/rkaldawy/Documents/scripts/terminal.sh",
                     selmon->sel->name, NULL};
  // char *command[] = {"/home/rkaldawy/Documents/scripts/terminal.sh", NULL};
  // char *command[] = {"urxvt", NULL};
  if (fork() == 0) {
    if (dpy)
      close(ConnectionNumber(dpy));
    setsid();
...
void tile(Monitor *m) {
  ...
  if (n > m->nmaster)
    ...
    mw = m->ww;
  for (i = 0, my = ty = topgappx, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++) {

    if (i < m->nmaster) {
      r = MIN(n, m->nmaster) - i;
      // h = (m->wh - my - gappx * (r - 1)) / r;
      h = (m->wh - my - gappx) * (c->cfact / mfacts);
      resize(c, m->wx + gappx, m->wy + my, mw - 2 * gappx - (2 * c->bw),
             h - (2 * c->bw), 0);
      my += HEIGHT(c) + gappx;
      mfacts -= c->cfact;
    } else {
      ...
      h = (m->wh - ty - gappx) * (c->cfact / sfacts);
      resize(c, m->wx + mw, m->wy + ty, m->ww - mw - gappx - (2 * c->bw),
             h - (2 * c->bw), False);
      ty += HEIGHT(c) + gappx;
      sfacts -= c->cfact;
    }
